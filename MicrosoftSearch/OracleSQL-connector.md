---
title: Connecteur SQL Graph Oracle pour Recherche Microsoft
ms.author: mecampos
author: mecampos
manager: umas
audience: Admin
...
# <a name="oracle-sql-graph-connector"></a>Connecteur SQL Graph Oracle

Le connecteur SQL Graph Oracle permet à votre organisation de découvrir et d’indexer des données à partir d’une base de données Oracle sur site. Le connecteur indexe le contenu spécifié dans Recherche Microsoft. Pour maintenir l’index à jour avec les données sources, il prend en charge des analyses complètes et incrémentielles périodiques. Avec le connecteur SQL Oracle, vous pouvez également restreindre l’accès aux résultats de recherche pour certains utilisateurs.
...
Cet article est réservé à toute personne qui configure, exécute et surveille un connecteur SQL Graph Oracle. Il complète le processus d’installation général et affiche des instructions qui s’appliquent uniquement au connecteur SQL Graph Oracle. Cet article inclut également des informations [sur la résolution des problèmes](#troubleshooting) et les [limitations.](#limitations)
...
Pour connecter votre connecteur SQL Oracle à une source de données, vous devez configurer le serveur de base de données que vous souhaitez analyser et l’agent Graph connecteur local. Vous pouvez ensuite vous connecter à la base de données avec la méthode d’authentification requise.
...
> [!NOTE]
> Votre base de données doit exécuter la base de données Oracle version 11g ou ultérieure pour que le connecteur puisse se connecter. Le connecteur prend en charge la base de données Oracle hébergée sur Windows, Linux et Azure VM platforms.
...
Une planification d’analyse complète recherche les lignes supprimées qui ont été précédemment synchronisées avec l’index Recherche Microsoft et toutes les lignes qui ont été déplacées hors du filtre de synchronisation. Lorsque vous vous connectez à la base de données pour la première fois, une analyse complète s’exécute pour synchroniser toutes les lignes récupérées à partir de la requête d’analyse complète. Pour synchroniser de nouvelles lignes et effectuer des mises à jour, vous devez planifier des analyses incrémentielles.
